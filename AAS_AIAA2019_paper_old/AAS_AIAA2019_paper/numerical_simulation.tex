...
%		\State $\phi_1$: 
		\If{$\phi > \phi_t$}
			\State Compute $t_1$, $t_2$, $t_f$ for $\phi > \phi_t$
			\State $t_1 = t_0 + \frac{\dot{\phi}_{max} - \dot{\phi}_0}{\ddot{\phi}_{max}}$
			\State $t_2 = t_1 + \frac{1}{\dot{\phi}_max} \big[ \phi_f - \dot{\phi}_0 (t_1 - t_0) - \frac{1}{2} \ddot{\phi}_{max} (t_f1 - t_0)^2 - \\ \frac{\dot{\phi}_{max} ( \dot{\phi}_{max} -  \dot{\phi}_f ) }{\ddot{\phi}_{max}} + \frac{ (\dot{\phi}_{max} - \dot{\phi}_f)^2}{2 \ddot{\phi}_{max}} \big]$
			\State $t_f=t_1+\frac{1}{\dot{\phi}_{max}}\Big[ \phi_f-\dot{\phi}_0(t_1-t_0)-\frac{1}{2}\ddot{\phi}_{max}(t_1-t_0)^2+\frac{(\dot{\phi}_{max}-\dot{\phi}_f)^2}{2\ddot{\phi}_{max}} \Big].$
		\Else 
			\State $ t_f = \sqrt{\frac{\dot{\phi}_{max}^2}{\ddot{\phi}_{max}}}$
			\State $t_2 = t_f / 2 $ 
			\State $t_1 = t_2 $ 
		\EndIf 
		\If{$\phi_2 > \phi_t$}
			\State Compute $t_1$, $t_2$, $t_f$ for $\phi > \phi_t$ 
		\Else 
			\State Compute $t_1$, $t_2$, $t_f$ for $\phi \leq \phi_t$
		\EndIf 
		\If{$\phi_3 > \phi_t$} 
			\State Compute $t_1$, $t_2$, $t_f$ for $\phi > \phi_t$ 
		\Else 
			\State Compute $t_1$, $t_2$, $t_f$ for $\phi \leq \phi_t$
		\EndIf 
		\For{$t$ between switching times}
			\State Find ${}^DR^N$
			\State Find ${}_\mathcal{B}\dot{\omega}^D$
			\State Solve for control torque $u$
			\State Propagate $\omega$ and $q$ 
		\EndFor 
%		\EndFor
	\end{algorithmic}
\end{algorithm}
...
%	\begin{algorithm} 
%		\caption{Computing Steering Profiles} 
%			\label{alg:pseudocode_steering}
%		\begin{algorithmic}[1]
%			\Function{test}
%		\end{algorithmic}
...
%	\subsection{SAS Algorithm Pseudocode: enumerate}
%	
%	\begin{enumerate}
%		
