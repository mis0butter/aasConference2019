...
\begin{document}
	...
	\author{
		Mohammad Ayoubi\thanks{Associate Professor, Department of Mechanical Engineering, Santa Clara University, Santa Clara, CA 95053. AIAA senior member, AAS senior member.} and Junette Hsin\thanks{Engineer, Dynamics and Control Analysis Group, Space Solutions (formerly Space Systems/Loral), postal address.}
	}
...
A novel geometric approach for a sun (or any bright object) avoidance slew maneuver with pointing and actuator constraints is presented. We assume spacecraft has a single light-sensitive payload with control-torque and reaction wheels' angular momentum constraints. Furthermore, we assume the initial and final attitudes, instrument boresight vector, and sun vector are known. Then we use Pontryagin's minimum principle (PMP) and derive the desired or target-frame quaternions, angular velocity and acceleration for two cases: 1) with control-torque and reaction wheels' angular momentum constraints, and 2) with control-torque constraints.  In the end, a Monte Carlo simulation is performed to show the viability of the proposed algorithm with control-torque and angular momentum constraints. 
...
%	\end{enumerate}
	...
	\begin{enumerate}
		...
		\phi_1=\left\{
		\begin{array}{ll}
		\cos^{-1}(\hat{P}\cdot_\mathcal{G}\hat{S}_{||})-\epsilon_p& when\  \cos^{-1}(\hat{P}\cdot_\mathcal{G}\hat{S}_{||})-\epsilon_p\leq \pi\\
		\cos^{-1}(\hat{P}\cdot_\mathcal{G}\hat{S}_{||})-\epsilon_p-2\pi& when\ \cos^{-1}(\hat{P}\cdot_\mathcal{G}\hat{S}_{||})-\epsilon_p>\pi\\
		\end{array}
		\right.
		\end{equation}
		\begin{figure}[htb]
			\begin{center}
				\includegraphics[width=3.5in]{./Figures/SVAS_1r}\includegraphics[width=3.5in]{./Figures/SVAS_1rb}
		...
		\begin{enumerate}
			...
			\begin{figure}[H]
				\begin{center}
					\includegraphics[width=3.5in]{./Figures/SVAS_2r}
			...
			\begin{figure}[H]
				\begin{center}
					\includegraphics[width=3.5in]{./Figures/SVAS_3r}
		...
		%		\end{enumerate}
		%		\begin{enumerate}[4]
		Similar to the $1^{st}$ maneuver, the vector $_\mathcal{N}\hat{P}_f$ needs to be transformed to the $\mathcal{G}$-frame before computing the dot product in Eq. (\ref{phi3_3}). 
		\item The final slew about the instrument boresight axis may be needed in order to go to the final attitude. 
	\end{enumerate}
	%	\end{enumerate}
	...
	{\it Problem Statement}: Consider the motion of a gyrostat around a given inertially-fixed axis, $_\mathcal{N}\hat{e}=[e_x,e_y,e_z]^T$. The problem of minimum-time slew maneuver around the $_\mathcal{N}\hat{e}$ axis can be formulated as:
	
	\begin{equation}\label{costfunction}
	\underset{u}{Minimize}\ \mathcal{J}[x(.), u(.), t_f]=\int_{t0}^{t_f} dt,
	\end{equation}
	...
	\begin{figure}[H]
		\begin{center}
			\includegraphics[width=3.5in]{./Figures/Spacecraft}  
